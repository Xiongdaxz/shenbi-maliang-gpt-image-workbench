...
const PLUGIN_NAME = "maliang-image-generator";
const MARKETPLACE_NAME = "maliang-internal";
...
export type LatestManifest = {
  ...
  update: {
    protocolVersion: 1;
    comparison: "semver";
    installSelector: typeof SELECTOR;
...
type CommandRunner = (command: string[], timeoutMs?: number) => Promise<CommandResult>;
...
function validateAutomaticUpdateUrl(url: URL, field: string) {
  if (url.username || url.password) throw new Error(`${field} contains credentials`);
  if (url.protocol === "https:") return;
  if (url.protocol === "http:" && privateOrLoopbackHostname(url.hostname)) return;
  throw new Error(`${field} must use HTTPS or trusted local/private-LAN HTTP`);
}

function parseTrustedUrl(value: unknown, field: string, expectedOrigin: string) {
  const url = new URL(requireString(value, field));
  validateAutomaticUpdateUrl(url, field);
  if (url.origin !== expectedOrigin) throw new Error(`Untrusted ${field} origin`);
  return url.href;
}

export function parseLatestManifest(value: unknown, expectedOrigin: string): LatestManifest {
  if (!isObject(value) || value.schemaVersion !== 1) throw new Error("Invalid update manifest schemaVersion");
  if (value.type !== "codex-plugin-marketplace") throw new Error("Invalid update manifest type");
  ...
  if (
    update.comparison !== "semver"
    || update.installSelector !== SELECTOR
    || update.strategy !== "transactional-local-marketplace-replacement"
    || update.defaultMode !== "auto"
    || update.automatic !== true
    || update.requiresUserApproval !== false
    || update.activation !== "next-task-or-restart"
  ) {
...
export async function readPluginManifest(pluginRoot: string): Promise<PluginManifest> {
  ...
  if (!isObject(value)) throw new Error("Invalid installed plugin manifest");
  const manifest = {
  ...
  if (manifest.name !== PLUGIN_NAME) throw new Error("Installed plugin name mismatch");
...
function checkUrlFromHomepage(homepage: string) {
  const origin = new URL(homepage).origin;
  return new URL(UPDATE_CHECK_PATH, origin).href;
}

export async function readUpdateResponseWithLimit(response: Response, maximumBytes: number) {
  if (!Number.isSafeInteger(maximumBytes) || maximumBytes < 0) throw new Error("Invalid response size limit");
  ...
  if (declaredLengthText) {
    ...
    if (!Number.isSafeInteger(declaredLength) || declaredLength < 0) throw new Error("Invalid Content-Length");
    if (declaredLength > maximumBytes) throw new Error("Response exceeds size limit");
  }
...
async function fetchBytes(url: string, timeoutMs: number, maximumBytes: number) {
...
async function fetchLatestManifest(checkUrl: string) {
  const origin = new URL(checkUrl).origin;
  const bytes = await fetchBytes(checkUrl, 10_000, MAX_MANIFEST_BYTES);
  return parseLatestManifest(JSON.parse(bytes.toString("utf8")) as unknown, origin);
}

async function defaultCommandRunner(command: string[], timeoutMs = COMMAND_TIMEOUT_MS): Promise<CommandResult> {
...
function parseCommandJson(result: CommandResult, label: string) {
...
function normalizeExtendedWindowsPath(value: string) {
...
async function getInstalledPlugin(runner: CommandRunner) {
  const value = parseCommandJson(await runner(["codex", "plugin", "list", "--json"]), "codex plugin list") as JsonObject;
  const installed = Array.isArray(value.installed) ? value.installed as InstalledPlugin[] : [];
  const plugin = installed.find((entry) => entry.pluginId === SELECTOR);
  if (!plugin) throw new Error(`Installed plugin ${SELECTOR} was not found`);
...
async function assertSafeTree(directory: string, totals = { bytes: 0, files: 0 }) {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (info.isSymbolicLink()) throw new Error(`Archive contains a symbolic link: ${entry.name}`);
    if (info.isDirectory()) await assertSafeTree(entryPath, totals);
    else if (info.isFile()) {
      totals.files += 1;
...
export async function validateMarketplaceRoot(
  ...
  if (
    ...
    || !rootEntries.every((entry) => entry.isDirectory() && [".agents", "plugins"].includes(entry.name))
  ...
  if (manifest.version !== expectedVersion) throw new Error("Archive plugin version mismatch");
  if (new URL(manifest.homepage).origin !== expectedOrigin) throw new Error("Archive plugin homepage origin mismatch");
  if (manifest.hooks !== "./hooks/hooks.json") throw new Error("Archive plugin Hook manifest is missing");
  ...
  if (
    !hookText.includes("^mcp__maliang__.*$")
    || !hookText.includes("auto-update.ts")
    || !hookText.includes("auto-update.mjs")
    || !hookText.includes("auto-update.cmd")
  ) {
  ...
  if (
    ...
    || localMcp.cwd !== "."
    || !Array.isArray(localMcp.args)
    || localMcp.args[0] !== "./mcp/maliang-local-mcp.mjs"
    || !Array.isArray(localMcp.enabled_tools)
    || !localMcp.enabled_tools.includes("upload_local_image")
    || !localMcp.enabled_tools.includes("save_image_result")
...
async function extractArchive(archivePath: string, destination: string, runner: CommandRunner) {
  ...
  const result = await runner(command, 60_000);
  if (result.exitCode !== 0) {
...
async function downloadAndStage(
  ...
  try {
    const archive = await fetchBytes(latest.downloadUrl, 45_000, MAX_ARCHIVE_BYTES);
    if (archive.length !== latest.size) throw new Error("Downloaded archive size mismatch");
...
async function verifyInstalledVersion(expectedVersion: string, runner: CommandRunner) {
...
async function refreshSelector(runner: CommandRunner) {
  const result = await runner(["codex", "plugin", "add", SELECTOR, "--json"], 60_000);
  if (result.exitCode !== 0) {
    throw new Error(`Plugin refresh failed: ${(result.stderr || result.stdout).trim().slice(0, 500)}`);
  }
}

async function installTransactionally(
  latest: LatestManifest,
  ...
  await validateMarketplaceRoot(
    marketplaceRoot,
    currentVersion,
    new URL(latest.downloadUrl).origin,
    latest.mcpResource
  );
  ...
  try {
    ...
    try {
      if (stagedMoved) await rename(marketplaceRoot, failedPath);
      if (currentMoved) await rename(backupPath, marketplaceRoot);
      if (currentMoved) await refreshSelector(runner);
      await rm(failedPath, { force: true, recursive: true }).catch(() => undefined);
    } catch (caught) {
    ...
    await rm(staged.extractionDirectory, { force: true, recursive: true }).catch(() => undefined);
    await rm(staged.archivePath, { force: true }).catch(() => undefined);
  }
}

function statePath(pluginData: string) {
  return path.join(pluginData, "update-state.json");
}

async function readState(pluginData: string): Promise<UpdateState> {
  try {
    const value = await readJson(statePath(pluginData));
    return isObject(value) && value.schemaVersion === 1 ? value as UpdateState : { schemaVersion: 1 };
  } catch (error) {
...
async function writeState(pluginData: string, state: UpdateState) {
...
async function logEvent(pluginData: string, message: string) {
  const logPath = path.join(pluginData, "auto-update.log");
  try {
    const info = await stat(logPath);
    if (info.size > 256 * 1024) await rename(logPath, `${logPath}.previous`).catch(() => undefined);
  } catch (error) {
...
async function readMode(pluginData: string): Promise<UpdateMode> {
  const environmentMode = String(process.env.MALIANG_PLUGIN_UPDATE_MODE ?? "").trim().toLowerCase();
  if (environmentMode) {
    if (environmentMode === "auto" || environmentMode === "notify" || environmentMode === "off") return environmentMode;
  ...
  try {
    ...
    if (isObject(value) && (value.mode === "auto" || value.mode === "notify" || value.mode === "off")) return value.mode;
...
function blockingOutput(reason: string) {
  return {
...
function printHookOutput(value: unknown) {
...
async function cleanupPreviousBackup(
  state: UpdateState,
  currentVersion: string,
  marketplaceRoot: string
) {
...
export async function runAutoUpdate(options?: {
  ...
  try {
    ...
    const result = await installTransactionally(
      latest,
      current.version,
      pluginData,
...
export async function main() {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    if (pluginData) {
      await mkdir(pluginData, { recursive: true }).catch(() => undefined);
      await logEvent(pluginData, `update-failed ${message}`).catch(() => undefined);
      const state = await readState(pluginData).catch((): UpdateState => ({ schemaVersion: 1 }));
      state.lastCheckAt = new Date().toISOString();
      state.lastErrorAt = state.lastCheckAt;
      state.lastError = message.slice(0, 1000);
