...
4. To edit a Maliang history image, pass its `imageId` through `imageIds` to `maliang_edit_image`. When the user requests multiple edited results, apply the same 1-through-10 image-count contract. Pass `background` and `outputFormat` when the user requests a transparent or opaque result; transparent output supports PNG or WebP, and continued edits should preserve an explicitly requested transparent background.
