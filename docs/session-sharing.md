...
创建结果中的绝对链接与插件安装、MCP/OAuth 共用全站公开访问地址。推荐在“后台 → 系统 → 品牌设置 → 站点访问设置”填写站点 origin，例如：

```text
https://image.example.com
```

保存后立即生效，无需重启。地址优先级依次为 `APP_PUBLIC_URL`、兼容环境变量 `MALIANG_PUBLIC_BASE_URL`、后台站点访问设置、请求 `Origin`、API 请求 origin。环境变量会接管并锁定后台输入框；非法地址或两个环境变量冲突会明确报错，不会静默回退。部署平台需要统一管理时仍可使用：

```env
APP_PUBLIC_URL=https://image.example.com
```

公开地址只能填写 HTTP(S) origin，不能包含 `/share`、`/install` 等路径、查询参数、锚点或账号密码。公网地址必须使用 HTTPS；本机和私有局域网地址可使用 HTTP。自动识别得到 `localhost`、`127.0.0.1` 或 `::1` 时，服务端会选择可用的非虚拟局域网 IPv4 替换主机名，并保留协议和端口。

- 本机开发：从 `localhost`、`127.0.0.1` 或 `::1` 创建分享时，返回链接会自动使用当前机器可用的局域网 IPv4；若机器没有可用的非回环 IPv4，才保留原地址。
- 局域网：自动选择结果不符合实际访问网卡时，可在后台配置 `http://192.168.x.x:<port>` 覆盖；同时确保服务监听可访问网卡，并开放系统防火墙端口。
- 公网：使用 HTTPS 反向代理，并在后台填写外部 HTTPS origin，或由部署平台配置 `APP_PUBLIC_URL`。若需信任代理传入的协议、Host 或真实访客地址，仅在反向代理是唯一入口且会覆盖客户端地址头时设置 `APP_TRUST_PROXY=true`。应用页面和 `/api` 应保持同源；当前绝对路由不支持子路径部署。
