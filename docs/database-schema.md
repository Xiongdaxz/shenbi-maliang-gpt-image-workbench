...
会话共享链接。创建分享时会按分享范围、素材可见策略、同一会话的消息 ID 和顺序查找完全一致的已有快照，命中后复用最早创建的原链接；只有这些条件发生变化时才新增记录并保存当时的标题。同一会话可保留多条不同快照，公开地址使用随机 UUID `public_token`，删除记录即立即撤销。旧版 HMAC 长链接仍可通过记录 ID校验访问。
...
| `include_references` | 是否允许共享页展示用户输入图、来源引用和结果关联引用，`0` 隐藏、`1` 保持现有分享行为 |
| `created_at` | 分享时间 |
...
灵感组与会话分享快照的关联。旧灵感不回填；只有作者明确勾选“分享会话”时创建。一个灵感组最多关联一个快照，同一个快照可以被多个灵感组复用。

| 字段 | 说明 |
| --- | --- |
| `group_id` | 灵感组 ID，主键 |
| `share_id` | `session_share_links.id`；分享记录撤销后级联删除关联 |
| `created_at` | 关联时间 |

`case_session_shares_share_idx` 支撑撤销与关联清理。删除灵感或来源图片时显式删除组关联，不自动撤销独立的会话分享链接。
