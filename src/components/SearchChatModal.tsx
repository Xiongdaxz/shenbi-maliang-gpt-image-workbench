    ...
    <div className="search-modal-backdrop" onMouseDown={onClose}>
      <section className="search-modal" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="search-modal-list">
          ...
          {searchKeyword && searchSessions.isFetching ? <div className="search-empty">正在搜索聊天...</div> : null}
          {groups.length === 0 && (!searchKeyword || !searchSessions.isFetching) ? <div className="search-empty">没有找到匹配的聊天</div> : null}
          {groups.map((group) => (
